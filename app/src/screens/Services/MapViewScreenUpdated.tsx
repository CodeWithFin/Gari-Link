...
// Import types for react-native-maps
type MapViewType = any;
type MarkerType = any;
type CalloutType = any;
type MapViewRefType = any;
...
try {
  ...
  console.warn('Failed to load react-native-maps module:', error);
}

const MapViewScreenUpdated: React.FC = () => {
  ...
  const [mapError, setMapError] = useState(!mapsAvailable);
  
  const mapRef = useRef<MapViewRefType>(null);
  ...
  useEffect(() => {
    ...
    const getUserLocation = async () => {
      try {
        ...
        // Animate map to user location if map is available
        if (mapRef.current && mapsAvailable) {
  ...
  const handleUserLocationPress = () => {
    if (userLocation && mapRef.current && mapsAvailable) {
      mapRef.current.animateToRegion({
        ...userLocation,
        latitudeDelta: 0.05,
        longitudeDelta: 0.05,
      ...
      <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
        <View style={styles.loadingContainer}>
          ...
          <View style={{ marginTop: 20 }}>
            <Text style={[styles.loadingText, { color: theme.colors.text, fontSize: 14, marginBottom: 10 }]}>
              For instructions on setting up Google Maps, see MAPS_SETUP.md in the project folder.
            </Text>
          </View>
        </View>
      </View>
...
export default MapViewScreenUpdated;
